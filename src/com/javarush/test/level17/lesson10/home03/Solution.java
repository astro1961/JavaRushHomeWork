...
public class Solution {
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        Thread man = new Thread(new Apteka.Person(), "Мужчина");
        Thread woman = new Thread(new Apteka.Person(), "Женщина");
        ...
        public void run()
        {
            while (!isStopped)
            {
                synchronized (this)
                {
                    ...
                    drugsController.buy(drug, count);
                    try
                    {
                        Thread.sleep(300);
                    }
            ...
            public void run()
            {
                ...
                while (!isStopped)
                {
                    synchronized (this)
                    {
                        Drug drug = getRandomDrug();
                        int count = getRandomCount();
                        drugsController.sell(drug, count);
                        waitAMoment();
                    }
                }
